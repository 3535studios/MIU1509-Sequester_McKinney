...
//Rare exceptions to indicate a bug in the _sandbox code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_sandbox = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_sandbox]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.sandbox.module"
