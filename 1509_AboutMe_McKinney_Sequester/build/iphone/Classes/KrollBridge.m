...
NSString * k1509_AboutMe_McKinney_Sequester$ModuleRequireFormat = @"(function(exports){"
...
@implementation k1509_AboutMe_McKinney_SequesterObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_k1509_aboutme_mckinney_sequester);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_k1509_aboutme_mckinney_sequester gc];
}
...
{
	// create k1509_AboutMe_McKinney_Sequester global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "k1509_AboutMe_McKinney_Sequester" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_k1509_aboutme_mckinney_sequester = [[k1509_AboutMe_McKinney_SequesterObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_k1509_aboutme_mckinney_sequester];
	
	NSString *_k1509_aboutme_mckinney_sequesterNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _k1509_aboutme_mckinney_sequesterNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_k1509_aboutme_mckinney_sequester valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:k1509_AboutMe_McKinney_Sequester$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.k1509_aboutme_mckinney_sequester.kroll" 
