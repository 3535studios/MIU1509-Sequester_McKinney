...
//Rare exceptions to indicate a bug in the _k1509_infoapp_mckinney_sequester code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_k1509_infoapp_mckinney_sequester = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_k1509_infoapp_mckinney_sequester]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.k1509_infoapp_mckinney_sequester.module"
