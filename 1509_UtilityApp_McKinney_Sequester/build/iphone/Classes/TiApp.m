...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kk1509_UtilityApp_McKinney_SequesterUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kk1509_UtilityApp_McKinney_SequesterUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a k1509_UtilityApp_McKinney_Sequester app using the node.js CLI
